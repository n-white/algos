...
BinaryHeap.prototype.insert = function (value) {
  ...
  // Escape function if parent is only node
  ...
  while (this._compare(this._heap[childIndex], this._heap[parentIndex])) {
  	...
  	parentIndex = Math.max(0, Math.floor( (childIndex - 1) / 2));
  }
}

BinaryHeap.prototype.removeRoot = function () {
  var newRoot = this._heap.pop();
  this._heap[0] = newRoot;
  var parentIndex = 0;
  var childIndex1 = parentIndex * 2 + 1
  var childIndex2 = parentIndex * 2 + 2

  while (this._compare(this._heap[parentIndex], this._heap[childIndex1]) || this._compare(this._heap[parentIndex], this._heap[childIndex2])) {
  	// Switch parent and child values
  	var parent = this._heap[parentIndex];
  	var child1 = this._heap[childIndex1];
  	var child2 = this._heap[childIndex2];
  	this._heap[parentIndex] = child;
  	this._heap[childIndex] = parent;
  	// Update the index numbers
  	parentIndex = childIndex;
	  childIndex1 = Math.max(parentIndex * 2 + 1, this._heap.length);
	  childIndex2 = Math.max(parentIndex * 2 + 2, this._heap.length);
  }
